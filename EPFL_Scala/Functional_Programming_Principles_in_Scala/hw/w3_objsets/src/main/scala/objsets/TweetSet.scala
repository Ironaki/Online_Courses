...
abstract class TweetSet {
  ...
  def mostRetweetedHelper(acc: Tweet): Tweet
...
class Empty extends TweetSet {
  ...
  def mostRetweetedHelper(acc: Tweet): Tweet = acc
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    if (p(this.elem)) leftRight.incl(this.elem)
    else leftRight
  }
  ...
  def mostRetweetedHelper(acc: Tweet): Tweet = {
    val larger = if (this.elem.retweets > acc.retweets) this.elem else acc
    left.mostRetweetedHelper(right.mostRetweetedHelper(larger))
  }
